...
Endless Lifecycle Support for Applications provides security fixes for the following open-source applications after official support ends:
...
Requests for customer-directed security patches for CVEs that are outside of the ELS for Applications scope will be reviewed within 3 working days. If the request is accepted, we will provide the patch within the next 60 days.
...
TuxCare's commitment to transparency and visibility is foundational to our ELS for Applications offering. We aim to provide comprehensive details about how each package is built, verified, and distributed, ensuring complete trust in the software supply chain.

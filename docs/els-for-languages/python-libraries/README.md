...
1. Create or update the `pip` configuration file and add the following:
   ...
   `[global]
   index-url = https://username:password@nexus-repo.corp.cloudlinux.com/repository/els-python/simple` },
...
1. Create or update the `pip` configuration file and add the following:
   ...
   { title: 'Linux/macOS (~/.pip/pip.conf)', content:
   `[global]
   extra-index-url = https://username:password@nexus-repo.corp.cloudlinux.com/repository/els-python/simple` },
   { title: 'Windows (%APPDATA%\pip\pip.ini)', content:
   `[global]
   extra-index-url = https://username:password@nexus-repo.corp.cloudlinux.com/repository/els-python/simple` }
   ]" />
